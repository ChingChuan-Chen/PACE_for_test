...
if p.regular == 3
    ...
    =PCAw(y,t,p.bwmu, p.bwmu_gcv,p.bwxcov, p.bwxcov_gcv,p.ntest1,p.ngrid1,p.selection_k, p.FVE_threshold, p.maxk,...
    p.control,p.regular,p.error,p.ngrid,p.method, p.ls_fit, p.shrink,p.newdata, p.kernel, p.numBins, p.yname, p.screePlot, p.designPlot, p.corrPlot, p.rho, p.verbose, p.xmu,p.xcov, p.method_mu, p.out_percent, p.weight,p.sigmat, p.varPositive);
