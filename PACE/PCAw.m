function [no_opt,sigma,lambda,phi,eigen,xi_est,xi_var,mu,muDense,bw_mu,xcov,bw_xcov, xcovfit, AIC,BIC,FVE,y_pred, y_predOrig, y_predDense, out1,out21,...
	  y,t, regular, rho_opt, sigmanew, mucopy, phicopy, eigencopy, out1copy, out21copy, xcovcopy, xcovfitcopy, xcorr, ops]... 
         =PCAw(y,t,bwmu,bwmu_gcv, bwxcov,bwxcov_gcv,ntest1,ngrid1,selection_k, FVE_threshold, maxk,...
              control,regular,error,ngrid,method,ls_fit,shrink,newdata,kernel, numBins,yname, screePlot, designPlot, corrPlot, rho, verbose, xmu, xcov, method_mu, out_percent, weight,sigmat, varPositive)
%===========
%Description:
%===========
%
...
%
%Input sigmat:    method to estimate sigma
%                 'constant':  using integration method to estimate variance of the measurement error, 
%                   that is, sigma is a constant.
%                 'tfcn'    :  estimate sigma using V(t)-G(t,t) without integration, that is, 
%                   sigma(t) is a function of time point t.
%                 either positive constant or 1*N vector whose elements are
%                 positive  :  a specified variance  of measurement error.
%========
% Details:
%========
...
%                 out1.
%
%Output mudense:  1*ngrid vector, similar to mu 
% 
...
if isempty(bwmu_gcv)
    bwmu_gcv = 1; % default bandwidth choice for mean function is GCV
end
if isempty(bwxcov)
...
yLength=cellfun(@(x) length(x), y); 
ynew=y(1,yLength~=0);    
tnew=t(1,yLength~=0); % this line aims at removing subjects with 0 measurement induced by userrange cut.

sigopt = {'constant', 'tfcn'};
if error==1
    if ischar(sigmat)%**
        [invalid, tmp_sig,xvar,yvar]=pc_covE_120823(t,out1,bw_xcov,ngrid,cut,kernel,rcov);%**
        sigid = strmatch(sigmat,sigopt,'exact');%**
        if sigid == 1;%**
            sigma = tmp_sig;%**
        elseif sigid == 2;%**
            sigma = yvar-xvar;%**
            ...
            id2 = find(sigma > 0);
            if ~isempty(id1)
                sigma(id1) = min(sigma(id2));
            end
        end%**
        if invalid==0
            [xi_est, xi_var, lambda, phi, eigen, no_opt, xcovfit,y_predOrig, rho_opt, sigmanew]=pc_est(ynew, tnew, mu, xcov, tmp_sig, no_opt, error, method, shrink, out1, out21,regular,rho, verbose);%**
            if sigid == 2 && strcmp(method,'CE')%**
                if length(sigma) ~= length(out1)
                    sigma = interp1(out21,sigma,out1,'spline');
                    id1 = find(sigma < 0);
                    id2 = find(sigma > 0);
                    if ~isempty(id1)
                        sigma(id1) = min(sigma(id2));
                ...
                [xi_est, xi_var,y_predOrig] = Scores1(ynew, tnew, mu, phi, lambda, sigma, sigma, no_opt, error, out1, regular);%** only for 'CE' method
                
            end%**
        else 
            xi_est=[]; xi_var=[]; lambda=[]; phi=[];
        end
    elseif isnumeric(sigmat)%**
        sigma = sigmat;%**
        [lambda, phi, eigen, no_opt] = getEigens(xcov,out1,out21,no_opt,1);
        ...
        if strcmp(method,'IN')
            if prod(size(sigma)) > 1%**
                ...
                tmp_sig = trapz(out21(ind1),sigma(ind1))*2/lint;
            else
                tmp_sig = sigma;
            end
            [xi_est, xi_var,y_predOrig, rho_opt]=getScores(ynew, tnew, mu, phi, lambda, tmp_sig, no_opt, error, method, shrink, out1, regular, rho, verbose);
        elseif strcmp(method,'CE')
            if prod(size(sigma)) == 1%**
                [xi_est, xi_var,y_predOrig, rho_opt]=getScores(ynew, tnew, mu, phi, lambda, sigma, no_opt, error, method, shrink, out1, regular, rho, verbose);
            elseif prod(size(sigma)) > 1%**
                [xi_est, xi_var,y_predOrig] = Scores1(y, t, mu, phi, lambda, sigma, sigma, no_opt, error, out1, regular);%** only for 'CE' method
            end%**
        end
        sigmanew = sigma;
        rho = -1;
...
if strcmp(method_mu,'RARE') == 1    % this part modified by Wenwen       
    ...
    if error==1
        [invalid, sigma]=pc_covE_120823(tnew,out1,bw_xcov,ngrid,cut,kernel,rcov);  %t=>tnew
        if invalid==0
...
% fit method 
if (ls_fit)
    ev_fit = fit_ev(rcov1, phi, out1, no_opt);
...
if invalid==0 
		y_pred=NaN(ncohort,length(mu));  
    y_predDense=NaN(ncohort,length(muDense));  %added by Wenwen
